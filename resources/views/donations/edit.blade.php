...
<body>
    ...
    <div class="d-flex flex-row">
        <aside class="d-flex flex-column flex-shrink-0 p-3 text-white bg-dark" style="width: 400px;">
            ...
            <form action="{{ route('donated-items.update', $donatedItem->id) }}" method="POST"
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="item-name" name="name" required
                        value="{{ old('name') ?? $donatedItem->name }}">
                    @error('name')
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="description" rows="3" name = "description" required>{{ old('description') ?? $donatedItem->description}}</textarea>
                    @error('description')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="item-location" name = "location" required
                        value="{{ old('location') ?? $donatedItem->location }}">
                    @error('location')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-3">
                    ...
                    <select class="form-select" name="category_id">
                        @foreach ($categories as $category)
                            @if (old('category_id', $donatedItem->category->id) == $category->id)
                ...
                <div class="mb-3">
                    ...
                    <input type="hidden" name="oldImage" value="{{ 'storage/' . $donatedItem->image }}">
                    @error('image')
                        <div class="text-danger">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="container mt-5" style="min-height: 100vh;">
            <div class="card" style="min-height: 50vh;">
                <div class="card-body">
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <img id="preview-image" class="img-fluid" src= "{{asset('storage/' . $donatedItem->image)}}">
                        </div>
                        <div class="col-md-6">
                            <h3 id="preview-name">Nama Barang : {{$donatedItem->name}}</h3>
                            <p class="font-weight-bold" id="preview-category">Kategori : {{$donatedItem->category->category_name}}</p>
                            <p class="font-italic" id="preview-description">Deskripsi : {{$donatedItem->description}}</p>
                            <p class="text-muted" id="preview-location">Lokasi: {{$donatedItem->location}}</p>
